...
class Simulation:
	...
	def print_op(self):
		...
		return str("{},{},{},{},{}".format(self.master_clock, self.next_arrival, self.queue[0].service_completeion_time if s.queue else '-', len(self.queue), self.retransmission_list))
...
with open(outfile, 'ab') as csvfile:
	...
	while s.master_clock <= s.master_clock_limit:
		s.simulate()
		# x = s.print_op()
		writer.writerow((s.master_clock, s.next_arrival, s.queue[0].service_completeion_time if s.queue else '-', len(s.queue), s.retransmission_list))
