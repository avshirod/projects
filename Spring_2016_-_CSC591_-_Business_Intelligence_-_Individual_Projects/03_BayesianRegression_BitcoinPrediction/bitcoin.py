...
def computeDelta(wt, X, Xi):
    ...
    # YOUR CODE GOES HERE
    dslen = len(X)-2
    meanx = np.mean(X[0:dslen])
    sdx = (X[0:dslen]).std()
    invariant = (X[0:dslen])-meanx
    num = 0.0
    denom = 0.0
    
    for i in range(0, len(Xi)):
        meanxi = np.mean(Xi.iloc[i, 0:dslen])
        sdxi = (Xi.iloc[i, 0:dslen]).std()
        similarity = sum(invariant * (Xi.iloc[i, 0:dslen] - meanxi)) / (sdxi * sdx * (dslen+1))
        t = np.exp(wt * similarity)
        num = num + t * Xi.iloc[i, dslen+1]
        denom += t
